...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($userOrEmail === '' || $password === '') {
        ...
        try {
            $stmt = $pdo->prepare(
                'SELECT id, username, nome, email, password, cargo, email_verified, approved FROM users WHERE username = :ue OR email = :ue LIMIT 1'
            );
            ...
            if ($user && password_verify($password, $user['password'])) {
                if ((int)($user['email_verified'] ?? 0) !== 1) {
                    $error = 'Por favor, confirme seu e-mail antes de acessar. Verifique sua caixa de entrada.';
                } elseif ((int)($user['approved'] ?? 0) !== 1) {
                    $error = 'Sua conta está aguardando aprovação administrativa. Você receberá um aviso assim que for liberado.';
                } else {
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: var(--font-family); }
    ...
    .form-control:not(:placeholder-shown) ~ .floating-label {
      ...
      background: var(--bg-dark); /* Blends with modal if transparent, but let's use a trick */
      padding: 0 6px;
      color: var(--accent);
      left: 40px;
    }
    
    /* We need a specific trick for the background of floating labels over glassmorphism:
       using the border color or a solid dark hex looks better */
    .form-control:focus ~ .floating-label,
    .form-control:not(:placeholder-shown) ~ .floating-label {
      background: #0b1325; 
      border-radius: 4px;
...
<body>
    ...
    <div class="glass-card stagger-in">
      ...
      <div class="brand-header stagger-in">
        <div class="logo-container">
          <!-- Pode usar a URL de logo original "assets/logo/logo-cores.png" -->
      ...
      <form method="post" novalidate class="stagger-in">
        <?php if ($next): ?>
        ...
        <div class="form-group">
          <input type="text" name="user" id="user" class="form-control" placeholder=" " required value="<?php echo htmlspecialchars($_POST['user'] ?? ''); ?>">
          ...
          <label for="user" class="floating-label">E-mail ou Usuário</label>
        </div>
  ...
  <script>
    ...
    // Adiciona foco inicial suave no input caso esteja vazio
